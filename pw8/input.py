...
class InputHandler:
    def __init__(self, menu, sms):
        ...
        self.lock = thrd.Lock() # for "dangerous" operations with threads
    ...
    def input_students(self, y):
        ...
        for i in range (student_amt):
            ...
            y -= 1
        t = thrd.Thread(target=self.sms.save_students, args=(1,))
        t.start()
        t.join()
        for i in range(y, y + 10):
            self.clearline(i)
    ...
    def input_courses(self, y):
        ...
        for i in range (course_amt):
            ...
            y -= 1
        t = thrd.Thread(target=self.sms.save_courses, args=(1,))
        t.start()
        t.join()
        for i in range(y, y + 10):
            self.clearline(i)
    ...
    def input_marks(self, y):
        ...
        t = thrd.Thread(target=self.sms.save_marks, args=(1,))
        t.start()
        t.join()
        y -= len(self.sms.courses) + 1
    ...
    def testinput(self, y):
        ...
        self.sms.save_students()
        self.sms.save_courses()
        self.sms.save_marks()
