...
class Course:
    ...
    def input(self):
        self.id = input("ID: ")
        self.name = input("Name: ")
        self.credits = int(input("Amount of credits: "))
    def __str__(self):
...
class SMS: # Student Management System
    ...
    def input_marks(self):
        ...
        self.calculate_avg_gpa() #  recalculating average GPA when the marks are changed
    ...
    def calculate_avg_gpa(self):
        if self.students == {} or self.courses == {} or self.marks == {}:
            print("Not enough data to calculate average GPA!")
            return
        for s in self.students:
            credits_list = []
            marks_list = []
    ...
    def display_avg_gpa(self):
        if self.avg_gpa == {}:
            self.calculate_avg_gpa()
        gpa_list = []
        for s in self.avg_gpa:
            gpa_list.append([s, self.avg_gpa[s]])
        gpa_list = np.array(gpa_list)
        indices = np.argsort(gpa_list[:,1], axis=0)[::-1]
        gpa_list = gpa_list[indices]
        print("> Students\' GPA sorted in descending order: ")
        for s in gpa_list:
            s_name = [i.name for i in self.students.values() if i.id == s[0]]
            print(f"{s_name}: {s[1]}")
    def testinput(self):
        if self.students != {} or self.courses != {}:
        ...
        self.students["789"] = Student("789", "Hoang", "05-11-1976")
        ...
        self.init_mark("p2", "789", 11.5)
        self.init_mark("i3", "789", 16.6)
        self.calculate_avg_gpa()
# main code
manager = SMS()
...
while True:
    ...
    print("7. Display sorted GPA list for all students")
    print("A. Create sample input (for quick testing)")
