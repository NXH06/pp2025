...
class InputHandler:
    ...
    def input_students(self, y):
        ...
        for i in range (student_amt):
            ...
            y -= 1
        with open("students.txt", "wb") as f: # for pickling, file must be opened in binary mode
            pickle.dump(self.sms.students, f) # just dump the object instead of having to format strings!
        for i in range(y, y + 10):
            self.clearline(i)
    ...
    def input_courses(self, y):
        ...
        with open("courses.txt", "wb") as f:
            pickle.dump(self.sms.courses, f)
        for i in range(y, y + 10):
            self.clearline(i)
    ...
    def testinput(self, y):
        ...
        with open("students.txt", "wb") as f:
            pickle.dump(self.sms.students, f)
        with open("courses.txt", "wb") as f:
            pickle.dump(self.sms.courses, f)
        with open("marks.txt", "wb") as f:
            pickle.dump(self.sms.marks, f)
    def read_from_file(self):
        try:
            with open("students.txt", "rb") as f:
                self.sms.students = pickle.load(f)
            with open("courses.txt", "rb") as f:
                self.sms.courses = pickle.load(f)
            with open("marks.txt", "rb") as f:
                self.sms.marks = pickle.load(f)
        except IOError:
